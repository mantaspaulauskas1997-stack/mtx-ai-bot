...
async def on_message(message: discord.Message):
    ...
    # Jeigu tai komanda su !, pirmiausia vykdom komandą
...
async def clear(ctx, amount: int = 10):
    ...
    if amount > 100:
        await ctx.reply("❌ Maksimaliai galima trinti 100 žinučių vienu metu.", mention_author=False)
        return

    try:
        deleted = await ctx.channel.purge(limit=amount + 1)

        msg = await ctx.send(f"🧹 Ištrinta žinučių: **{len(deleted) - 1}**")
        await msg.delete(delay=5)

    except discord.Forbidden:
        await ctx.reply("❌ Botas neturi teisės trinti žinučių.", mention_author=False)
    except Exception as e:
        await ctx.reply(f"❌ Klaida trinant žinutes: {e}", mention_author=False)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.reply("❌ Tu neturi reikalingų teisių šiai komandai.", mention_author=False)

    elif isinstance(error, commands.BotMissingPermissions):
        await ctx.reply("❌ Botui trūksta teisių šiai komandai.", mention_author=False)

    elif isinstance(error, commands.BadArgument):
        await ctx.reply("❌ Blogas formatas. Naudok taip: `!clear 10`", mention_author=False)

    elif isinstance(error, commands.CommandNotFound):
        return

    else:
        await ctx.reply(f"❌ Komandos klaida: {error}", mention_author=False)
